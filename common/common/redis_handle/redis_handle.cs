...
namespace abelkhan
{
    public class redis_handle
    {
        ...
        public Task<bool> SetStrData(string key, string data, long timeout = constant.constant.DefaultRedisCacheTimeout)
        {
            while (true)
            {
                try
        ...
        public Task<bool> SetData<T>(string key, T data, long timeout = constant.constant.DefaultRedisCacheTimeout)
        {
        ...
        public async Task<bool> LPushData<T>(string key, T data)
        {
            await database.ListLeftPushAsync(key, JsonConvert.SerializeObject(data));
            return true;
        }

        public async Task<T> RPopData<T>(string key)
        {
            string json = await database.ListRightPopAsync(key);
            if (string.IsNullOrEmpty(json))
            {
                return default(T);
            }
